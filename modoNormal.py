...
# Cambiar proxy a "ON" (proxied=True) salvo los excluidos
for record in data["result"]:
    ...
    if not record["proxied"]:  # Solo modificar si NO está ya proxied
        dns_id = record["id"]
        ...
        if update_data["success"]:
            print(f"Registro {nombre} actualizado a PROXIED (CDN ON)")
        else:
            print(f"Error al actualizar {nombre}: {update_data}")
    else:
        print(f"⚡ Registro {nombre} ya está en PROXIED")
